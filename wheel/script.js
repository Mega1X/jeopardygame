...
const segments = [
    ...
    { color: "#FFD700", text: "$1000", value: 1000 },
...
function renderBoard() {
    ...
    words.forEach(word => {
        let currentLen = lines[lineIndex].reduce((acc, w) => acc + w.length + 1, 0) - 1;
    ...
    else if (usedLines === 3) startRow = 0;
...
function handleGuess(letter, btn) {
    if (btn.classList.contains('used')) return; // Extra safety check

    btn.disabled = true;
    btn.classList.add('used'); // Mark as permanently used
...
function enableKeyboard() {
    ...
    keys.forEach(k => {
        // IMPORTANT: Only re-enable if NOT used!
